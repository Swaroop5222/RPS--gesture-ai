...
pygame.mixer.init()
win_music=pygame.mixer.Sound('game_won.wav')
lose_music=pygame.mixer.Sound('game_lost.wav')
...
while True:
   ...
   cv.putText(frame,f"Comment: {comment}",(20,140),cv.FONT_HERSHEY_DUPLEX,1,(0,255,255),2,cv.LINE_AA)
   ...
   if key==ord('q'):
      ...
      if result in ["You Win!", "Computer Wins!", "It is a Tie!"]:
         comment = generate_commentary(player_gesture, computer_gesture, result)
         if len(comment) > 45:
            comment = comment[:45] + "..."
